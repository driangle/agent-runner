...
- [ ] Implement `Run(ctx context.Context, prompt string, opts ...Option) (*Result, error)` using functional options pattern
- [ ] Build CLI arguments from options (`--print`, `--output-format stream-json`, `--model`, etc.)
- [ ] Spawn process with `exec.CommandContext`, pipe stdout, capture stderr
- [ ] Scan stdout line by line, parse each as a stream-json message
- [ ] Extract the final `result` message for the return value
- [ ] Handle context cancellation (deadline → `ErrTimeout`)
- [ ] Handle non-zero exit codes with stderr in error message
- [ ] Support `CommandBuilder` injection for testability (like modpol)
- [ ] Default binary to `"claude"`, allow override via option
